...
async function deleteGame() {
  if (!props.game) return;
  const ok = window.confirm(`确认删除“${props.game.title}”的条目？\n\n不会删除游戏文件。`);
  if (!ok) return;
...
function openUserDataDir() {
  if (!containerInfo.value) return;
  emit("openPath", containerInfo.value.userDataDir);
}
</script>
      ...
      <Card class="w-full max-w-lg p-4">
        ...
        <div class="mt-4 space-y-3">
          ...
          <div class="grid grid-cols-1 gap-2 sm:grid-cols-2">
            ...
            <Button variant="secondary" class="justify-start" @click="openUserDataDir">
              <Icon icon="ri:folder-user-line" class="size-4" />
              打开 User Data
            </Button>
          </div>
          ...
          <div class="flex items-center justify-between">
            ...
            <Button variant="destructive" @click="deleteGame">
