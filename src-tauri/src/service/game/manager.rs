...
use crate::model::{AddGameInput, EngineType, GameDto, UpdateGameInput};
use crate::service::fs::FileService;
...
impl GameService {
    ...
    pub async fn get_game_by_path(&self, path: &str) -> Result<Option<Game>, String> {
        let normalized_input = crate::util::path::canonicalize(std::path::Path::new(path))
            .to_string_lossy()
            .to_string();
        ...
        for g in all_games {
            let g_norm = crate::util::path::canonicalize(std::path::Path::new(&g.game_path))
                .to_string_lossy()
                .to_string();
    ...
    pub async fn add_game(&self, input: AddGameInput) -> Result<Game, String> {
        // 规范化路径并检查是否已存在（按路径）
        let normalized_path = crate::util::path::canonicalize(Path::new(&input.path))
            .to_string_lossy()
            .to_string();
    ...
    pub async fn update_game(&self, id: &str, input: UpdateGameInput) -> Result<Game, String> {
        ...
        if let Some(ref path) = input.path {
            // 规范化并保存路径，并检查冲突
            let normalized = crate::util::path::canonicalize(Path::new(path))
                .to_string_lossy()
                .to_string();
        ...
        game.updated_at = crate::util::now_unix_ms();
    ...
    pub async fn update_cover_path(
        ...
        game.update()
            ...
            .updated_at(crate::util::now_unix_ms())
            .exec(&mut *db)
            .await
