...
use crate::model::{EngineType, GameConfig, LaunchResult};
use std::path::{Path, PathBuf};
use std::process::{Child, Command};

/// 游戏启动服务
pub struct LauncherService {
    file_service: crate::service::fs::FileService,
}
...
impl LauncherService {
    ...
    pub fn new() -> Self {
        Self {
            file_service: crate::service::fs::FileService::new(),
        }
    }
    ...
    pub async fn launch_game(
        ...
        if !game_path.exists() {
            return Err(format!("游戏路径不存在: {}", game.game_path));
        }
    ...
    async fn launch_other_game(
        ...
        if options.use_bottles {
            ...
            #[cfg(target_os = "linux")]
            {
                use crate::service::extension::BottlesService;
